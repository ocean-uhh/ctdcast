...
.. list-table::
   ...
   * - ``clock.segments``
     - Stage-2 acquisition-clock correction: a list of
       ...
       (``NMEA - System``).  **Generated by ``ctdcast clock``** — paste it in as-is
       rather than hand-computing the sign.  Applied only where the coordinate is on
       the System clock (a cruise already on GPS is refused); a ``drift`` verdict has
       no applier.
