...
def test_registry_loads_known_sensor_ids() -> None:
    ...
    reg = _registry()
    known = ("55", "3", "45", "38", "0", "20", "67", "71", "43", "61",
             "11", "13", "42", "51", "46", "58")
    for sid in known:
...
def test_known_sensor_resolves_from_registry_alone() -> None:
    ...
    with warnings.catch_warnings():
        ...
        attrs = resolve_sensor(
            sensor_id="55", serial="4823", role="temperature_1",
            calibration_date="2021-05-07", element="TemperatureSensor", registry=reg,
        )
...
def test_role_override_sharpens_generic_default() -> None:
    ...
    attrs = resolve_sensor(
        sensor_id="20", serial="FLNTURTD-3219", role="fluorometer",
        registry=reg, overrides=ov,
    )
...
def test_altimeter_unresolved_warns_but_does_not_raise() -> None:
    ...
    with pytest.warns(UserWarning, match=r"sensors\.overrides\.altimeter"):
        attrs = resolve_sensor(
            sensor_id="0", serial="42299", role="altimeter", registry=reg,
        )
...
def test_altimeter_resolved_by_override() -> None:
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("error")
        attrs = resolve_sensor(
            sensor_id="0", serial="42299", role="altimeter", registry=reg, overrides=ov,
        )
...
def test_cruise_override_fills_unknown_sensor_id_silently() -> None:
    ...
    ov = SensorOverrides.from_cruise_config({
        "sensors": {"overrides": {"temperature_1": {
            "sensor_model": "SBE 3F",
    ...
    with warnings.catch_warnings():
        ...
        attrs = resolve_sensor(
            sensor_id="999", serial="7321", role="temperature_1",
            registry=reg, overrides=ov,
        )
...
def test_unknown_sensor_id_without_override_still_warns() -> None:
    ...
    with pytest.warns(UserWarning, match="Unknown SeaBird SensorID"):
        attrs = resolve_sensor(sensor_id="999", serial="7321", role="temperature_1",
                               registry=reg)
    assert attrs["sensor_model"] == "UNK"
...
def test_strict_raises_on_unresolved() -> None:
    ...
    with pytest.raises(ValueError, match="not resolved"):
        resolve_sensor(sensor_id="0", serial="42299", role="altimeter",
...
def test_unknown_sensor_id_warns_and_returns_unk() -> None:
    ...
    reg = _registry()
    with pytest.warns(UserWarning, match="Unknown SeaBird SensorID"):
        attrs = resolve_sensor(sensor_id="999", serial="1", role="mystery", registry=reg)
    assert attrs["sensor_model"] == "UNK"
...
def test_catalog_var_name_and_sanitize() -> None:
    """Catalog names follow SENSOR_<TYPE>_<INDEX>_<SERIAL> with OG1 sanitizing."""
    ...
    assert catalog_var_name("fluorometer", "FLNTURTD-3219") == "SENSOR_FLUOROMETER_1_FLNTURTD_3219"
...
def test_element_mismatch_warns() -> None:
    ...
    with pytest.warns(UserWarning, match="disagrees with registry element"):
        resolve_sensor(sensor_id="55", serial="1", role="temperature_1",
                       element="TurbidityMeter", registry=reg)
