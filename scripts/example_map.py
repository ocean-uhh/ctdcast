...
if _ACTIVITIES_YAML.exists():
    ...
    print(f"WARNING: {_ACTIVITIES_YAML} not found — mooring positions will not be plotted")
...
if _GEBCO.exists():
    ...
    ax_sm.contourf(_blon2, _blat2, _dep2,
                   levels=np.arange(0, 2001, 100),
                   cmap="Blues", alpha=0.35, vmin=0, vmax=2000)
    ax_sm.contour(_blon2, _blat2, _dep2,
                  levels=[200, 500, 700, 1000, 1500],
                  colors="steelblue", linewidths=0.4, alpha=0.6)
else:
...
for _moor in _moorings_sm:
    ax_sm.plot(_moor["lon"], _moor["lat"], "^", color=MOORING_COLOR,
               ms=10, mfc="white", mew=1.5, zorder=2)
    ax_sm.annotate(_moor["label"].split("_")[0], (_moor["lon"], _moor["lat"]),
                   xytext=(5, 4), textcoords="offset points",
                   fontsize=7, color=MOORING_COLOR, fontweight="bold", zorder=3)
...
for _grp in _clusters:
    ...
    ax_sm.plot(_clon_c, _clat_c, _marker, color=_color,
               ms=7, mew=_mew, alpha=0.85, zorder=4)
    ax_sm.annotate(
        _fmt_cluster_label(_casts), (_clon_c, _clat_c),
        xytext=(4, 0), textcoords="offset points",
        fontsize=5, color="#333333", zorder=5, va="center",
    )

# Legend — lower right
_handles = [mpatches.Patch(color=col, label=sec) for sec, col in _section_colors.items()]
_handles.append(mpatches.Patch(color="#aaaaaa", label="unassigned (not in YAML)"))
_handles.append(mlines.Line2D([0], [0], marker="^", color=MOORING_COLOR, mfc="white",
                               ms=8, lw=0, label="mooring"))
ax_sm.legend(handles=_handles, loc="lower right", fontsize=8,
             framealpha=0.85, title="Section", title_fontsize=9)
