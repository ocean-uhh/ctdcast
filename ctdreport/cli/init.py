...
"""
    ...
    parser.add_argument(
        ...
        help=(
            "Maximum number of casts in a single section group.  Groups larger than "
            "this are split into consecutive chunks.  Default: 25."
        ),
    )
    return parser
    ...
    if not profiles_nc:
        ...
        if not profiles_path.exists():
            ...
            if ans not in {"n", "no"}:
                ...
                print(
                    f"\nDetection thresholds (--dx-timeseries {dx_ts} km, --dx-section {dx_sec} km)"
                )
                try:
                ...
                print(
                    ...
                    f" max_section_casts={max_sec})..."
                )
                try:
                    sections, timeseries = _detect_groups(
                        profiles_path, dx_ts, dx_sec, max_sec
                    )
                ...
                if resolved is not None:
                    yaml_text = _format_sections_yaml(
                        sections, timeseries, dx_ts, dx_sec, max_sec
                    )
...
def _detect_groups(
    ...
    Consecutive casts separated by more than *dx_section_km* km start a new
    group.  Groups with more than *max_section_casts* casts are further split
    into consecutive chunks of at most that size.  Within each group the
    straight-line span (first-to-last cast distance) determines classification:
    span ≤ *dx_timeseries_km* → timeseries (repeat station); span >
    *dx_timeseries_km* → section (transect).
    ...
    dx_section_km:
        Inter-cast distance (km) that triggers a section break (transit).
    max_section_casts:
        Maximum number of casts in a single group before it is split into
        consecutive chunks.  Default: 25.
    ...
    # Split into groups wherever consecutive inter-cast gap > dx_section_km.
    ...
    # Further split any group that exceeds max_section_casts.
    raw_groups: list[list[int]] = []
    for g in gap_groups:
        if len(g) <= max_section_casts:
            raw_groups.append(g)
        else:
            for chunk_start in range(0, len(g), max_section_casts):
                raw_groups.append(g[chunk_start : chunk_start + max_section_casts])
...
def _format_sections_yaml(
    ...
    lines: list[str] = [
        ...
        (
            ...
            f" dx_section={dx_section_km} km, max_section_casts={max_section_casts}"
        ),
